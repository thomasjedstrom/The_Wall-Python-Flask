...
def delete():
    ...
    query_data = { 'message_id': request.form['action']}
    mysql.query_db(delete_comment_query, query_data)

    delete_message_query = "DELETE FROM messages WHERE id = :message_id"
    query_data = { 'message_id': request.form['action']}
    mysql.query_db(delete_message_query, query_data)
    return redirect('/wall')
